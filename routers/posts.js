...
router.get('/posts', async(ctx, next) => {
    ...
    if (ctx.request.querystring) {
        ...
        let result = await userModel.findDataByUser(name)
        postsLength = result.length
        await ctx.render('selfPosts', {
            session: ctx.session,
            posts: await userModel.findPostByUserPage(name, 1),
            postsPageLength: Math.ceil(postsLength / 10),
        })
    } else {
        let result = await userModel.findAllPost()
        postsLength = result.length
        await ctx.render('posts', {
            session: ctx.session,
            posts: await userModel.findPostByPage(1),
...
router.post('/posts/page', async(ctx, next) => {
    ...
    try {
        let result = await userModel.findPostByPage(page)
        ctx.body = result 
    } catch (error) {
        ctx.body = 'error'
    }
...
router.post('/create', async(ctx, next) => {
   ...
   let res = await userModel.findUserData(ctx.session.user)
   console.log(res[0]['avator'])
   avator = res[0]['avator']
   try {
    userModel.insertPost([name, newTitle, md.render(content), content, id, time, avator])
    ctx.body = true
   } catch(e) {
    ctx.body = false
   }
